...
class Conv2D(Layer):
    ...
    def feed_forward(self, X):
        ...
        out = wrow @ self.X_cols + self.bias
        #out = np.dot(wrow, self.X_cols) + self.bias

        # output shape is (depth, hout, wout)
    ...
    def backpropagate(self, delta):
        ...
        delta_kernel = (delta_flat @ self.X_cols.T).reshape(self.kernels.shape)
        ...
        # convert to original input dimension
...
class ConvMono(Layer):
    ...
    def backpropagate(self, dout):
        """
            Backpropagate in the conv layer:
                dW = convolve(delta, X)
                dX (change in input) = convolve(delta, W)
        """
